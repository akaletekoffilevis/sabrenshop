export const WHATSAPP_NUMBER =
  process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "22789148454";
...
export function productWhatsappMessage(opts: {
  ...
  const variant =
    [opts.color, opts.size].filter(Boolean).join(" / ") || "Standard";
...
function customerLines(customer?: CartCustomer | null): string[] {
  ...
  if (customer.phone?.trim())
    parts.push(`Téléphone : ${customer.phone.trim()}`);
  if (pref !== "pickup") {
    const where = [customer.quartier, customer.city]
      .filter(Boolean)
      .map((s) => s?.trim())
      .join(", ");
    if (where) parts.push(`Localisation : ${where}`);
    if (customer.address?.trim())
      parts.push(`Point de repère : ${customer.address.trim()}`);
  }
...
export function cartWhatsappMessage(
  ...
  if (opts.discount && opts.discount > 0)
    parts.push(
      `Remise (${opts.promoCode || "code"}) : -${opts.discount.toLocaleString("fr-FR")} FCFA`,
    );

  // if (opts.isPickup) parts.push(`Mode : Retrait boutique (0 FCFA)`);
  // else
  //   parts.push(
  //     opts.deliveryFee && opts.deliveryFee > 0
  //       ? `Livraison : ${opts.deliveryFee.toLocaleString("fr-FR")} FCFA`
  //       : `Livraison : Offerte`,
  //   );
