          ...
          <div className="md:hidden space-y-3">
            {products.map((p) => (
              <Card key={p.id} className="p-3 flex items-center gap-3">
                <Link href={`/admin/produits/${p.id}`} className="shrink-0">
                  <span className="w-16 h-16 rounded-xl overflow-hidden bg-sabren-cream border border-sabren-gray flex items-center justify-center text-sabren-black/20">
                    {(p.images as string[])[0] ? (
                      <img src={(p.images as string[])[0]} alt={p.name} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                    ) : (
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/admin/produits/${p.id}`} className="font-bold text-sm line-clamp-1 hover:text-sabren-gold transition">
                    {p.name}
                  </Link>
                  <p className="text-[11px] text-sabren-black/40 mt-0.5">{p.category?.name ?? "Non classé"}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <span className="font-black text-sm">{formatPrice(p.price)}</span>
                    <Badge tone={p.stock === 0 ? "red" : p.stock < 5 ? "gold" : "green"}>{p.stock === 0 ? "Rupture" : `${p.stock} en stock`}</Badge>
                  </div>
                </div>
