...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const dup = await prisma.promoCode.findFirst({ where: { code: parsed.data.code, id: { not: id } } });
  if (dup) return Response.json({ error: "Ce code existe déjà." }, { status: 409 });

  const promo = await prisma.promoCode.update({ where: { id }, data: { ...parsed.data, expiresAt: parsed.data.expiresAt } as any });
