  ...
  return (
    <section className="max-w-7xl mx-auto container-px lg:px-8 pt-12">
      <div className="flex items-end justify-between mb-6">
        ...
        <Link href="/boutique" className="inline-flex items-center gap-1.5 text-sm font-semibold text-sabren-black/70 hover:text-sabren-gold transition">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4">
        {categories.map((c) => (
          <Link
            ...
            className="group relative rounded-2xl overflow-hidden bg-white border border-sabren-gray shadow-card hover:shadow-card-hover transition aspect-[4/5] block"
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-3">
              <h3 className="font-display font-bold text-white text-[13px] md:text-sm line-clamp-2 group-hover:text-sabren-gold transition">{c.name}</h3>
              <span className="inline-flex items-center gap-1 mt-1.5 text-[11px] font-bold text-sabren-gold opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition hidden md:inline-flex">
                Découvrir
              </span>
            </div>
            <span className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/90 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition hidden md:flex">
