...
export async function CategoryGrid() {
  let categories: any[] = [];
  try {
    categories = await prisma.category.findMany({ where: { isActive: true }, orderBy: { position: "asc" } });
  } catch {}
  ...
  return (
    <section className="max-w-7xl mx-auto container-px lg:px-8 pt-12">
      <div className="flex items-end justify-between mb-6">
        <div>
          <span className="text-xs font-bold uppercase tracking-[0.18em] text-sabren-gold">Explorez la boutique</span>
        ...
        {categories.map((c) => (
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-3">
              <h3 className="font-display font-bold text-white text-sm group-hover:text-sabren-gold transition">{c.name}</h3>
              <p className="text-[11px] text-white/70 line-clamp-1">{c.description}</p>
