...
export function ProductReviews({ productId, initial }: { productId: string; initial: Review[] }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReviews((r) => [{ id: data.review.id, name: data.review.name, rating: data.review.rating, comment: data.review.comment, createdAt: data.review.createdAt }, ...r]);
      setSent(true);
      setName("");
      setComment("");
      setRating(5);
    ...
    <div className="grid lg:grid-cols-3 gap-6 mt-12 items-start">
      ...
      <div className="bg-white rounded-2xl border border-sabren-gray shadow-card p-6 lg:sticky lg:top-28">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {sent && <p className="text-xs font-semibold text-green-600 bg-green-50 rounded-xl px-3 py-2 flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> Merci ! Votre avis est publié.</p>}
