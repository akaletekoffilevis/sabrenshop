      ...
      {products.length === 0 ? null : (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <p className="text-xs font-bold uppercase tracking-wide text-sabren-black/45">
              {products.length} produit{products.length > 1 ? "s" : ""} au catalogue
            </p>
            <div className="flex items-center gap-2">
              <Link href="/admin/produits/nouveau" className="md:hidden inline-flex items-center gap-1.5 bg-sabren-gold text-sabren-black font-bold rounded-full px-4 py-2 text-xs hover:bg-sabren-gold-hover transition">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
              {paged.map((p) => (
                <Card key={p.id} className="overflow-hidden flex flex-col">
                  ...
                  <div className="flex items-center justify-between border-t border-sabren-gray/60 px-3 py-2 mt-auto">
                    <Link href={`/admin/produits/${p.id}`} className="text-[10px] font-bold uppercase text-sabren-gold hover:underline">
                      Modifier
                    </Link>
                    <DeleteProductButton id={p.id} name={p.name} />
                  </div>
