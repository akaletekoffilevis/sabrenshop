...
export function AdminSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
      ...
      {adminLinks.map((l) => {
        ...
        return (
          <Link
            ...
            href={l.href}
            title={l.label}
            className={`flex items-center gap-3 px-2.5 py-2.5 rounded-xl text-sm font-medium transition ${
  ...
  const bottom = (
    <div className="space-y-1 px-2 py-3 md:px-3 border-t border-white/10">
      <Link href="/" title="Voir la boutique" className={`flex items-center gap-3 px-2.5 py-2.5 rounded-xl text-sm text-white/70 hover:bg-white/10 transition ${collapsed ? "justify-center" : "justify-start"}`}>
      ...
      <button
        ...
        className={`flex items-center gap-3 px-2.5 py-2.5 rounded-xl text-sm text-white/70 hover:bg-red-500/20 hover:text-red-300 transition w-full ${collapsed ? "justify-center" : "justify-start"}`}
      >
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-40 bg-sabren-black text-white flex-col hidden md:flex overflow-hidden transition-[width] duration-300 ease-in-out ${
        collapsed ? "w-[68px]" : "w-64"
      }`}
    >
      <div className={`flex items-center border-b border-white/10 px-2 py-4 ${collapsed ? "justify-center" : "justify-between"}`}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className={`leading-tight whitespace-nowrap transition-opacity ${collapsed ? "hidden" : "block"}`}>
        ...
        <button
          onClick={onToggle}
          className="inline-flex p-2 rounded-lg hover:bg-white/10 transition text-white/60 hover:text-white"
